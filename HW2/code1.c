...
void printHistogram(int binArray[], int num_bins, int bin_size);
...
char directoryPathName[50];
// declare the variable to hold the bin sizeint bin_size = 0, num_bins;
...
scanf("%s", directoryPathName);
// check the condition whether an object can be created
// with the given directoryPathName
if (stat(directoryPathName, &stat_struct) == -1)
{
perror("The given input is invalid");
exit(EXIT_FAILURE);
}
// check the condition that whether the given
// directoryPathName is a directory
...
func_ReturnValue = decend_Tree_Dir(filesList.start_ptr, directoryPathName);
...
num_bins = (int)ceil(max_Size / bin_size) + 1;
// declare an array of bins
int bin_arr[num_bins];
...
// bins
upDateBin(statTreeNode, bin_arr, bin_size);
// re-set the statTreeNode
statTreeNode = filesList.start_ptr;
// call the function printHistogram() to diaply the histogram
printHistogram(bin_arr, num_bins, bin_size);
return 0;
}

void printHistogram(int bin_arr[], int num_bins, int bin_size)
{puts("Histogram of the given directory tree and bin width:");
for (int i = 0; i < num_bins; i++)
{
printf("%5d - %5d\t| ", bin_size * i, bin_size * (i + 1));
for (int j = 0; j < bin_arr[i]; j++)
{
printf("%s", "*");
}
printf("\n");
...
int file_status = -1;// declare a variable to hold the error status
int err_status = -1;
...
err_status = errno;// free the stat_buffer
free(stat_buffer);
